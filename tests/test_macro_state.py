...
=========================
Гейтовете на api export-а (мандат ORGANISM-v1, bg огледалото на jp Ф1):
фамилната схема + тъждеството „api ≡ последния ред на журнала" + режимът ≡
get_regime. Ако api файлът и журналът се разминат, дрилът и сателитът
тръгват от лъжа.
"""
import json
...
def test_top_level_carries_the_family_keys(state):
    ...
    executive_summary · lenses. BG добавя слоевете си, не маха фамилните."""
    doc, *_ = state
...
def test_every_lens_quotes_the_journal_scores(state):
    ...
    for lens, block in doc["lenses"].items():
        assert block["score"] == (None if pd.isna(row[f"score_{lens}"])
                                  else float(row[f"score_{lens}"])), lens
        assert block["health_z"] == (None if pd.isna(row[f"z_{lens}"])
                                     else float(row[f"z_{lens}"])), lens
...
def live_state(tmp_path_factory):
    ...
    # Живата верига (мандат №54): splice → derive; пълнотата се брои СЛЕД нея.
...
def test_the_live_api_regime_matches_the_journal_composite(live_state):
    ...
    консуматорът никога не дублира праговете (мотивът на мандата)."""
    doc, journal = live_state
