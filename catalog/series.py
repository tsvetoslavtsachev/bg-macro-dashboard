...
# `derived` НЕ е източник на данни, а декларация: серията се РАЖДА от снапшота
# (мандат №54). Никой адаптер не я фетчва — `build_adapters()` няма такъв ключ,
# затова `_build_snapshot` мълчаливо я подминава, а `sources/derived.py` я
# произвежда СЛЕД сплайса. Флагът стои тук, за да не се промъкне изведена серия
# като „забравена за фетчване" — валидаторът я приема само защото я декларираме.
...
SERIES_CATALOG: dict[str, dict[str, Any]] = {
    ...
    # ── Номиналният БВП — ТРЕТАТА контекстна серия (мандат №54) ──────────────
    # Знаменателят на кредитния въпрос. Котвата на №47 („кредит трайно над
    # номиналния ръст надува баланси, не производство") се цитираше от ръчна
    # снимка; тук тя става ВИДИМА на таблото и жива в експорта.
    # Защо NSA, а не SCA: конвенцията на изведената серия отдолу е 4-тримесечна
    # СУМА, а тя сама гаси сезонността — сезонното изглаждане отгоре би било
    # второ изглаждане на същото (живо проверено 29.07.2026: SCA дава същото
    # ratio до 0.1пп). Суровото е НИВО в млн. EUR, затова `is_rate: False` —
    # ставка става чак СЛЕД трансформацията, точно както при `BG_PERMITS`.
    # `yoy_roll4` (4-тримесечната плъзгаща на г/г темпа) е и прочитът, който
    # стои до кредитния г/г в разрива: две изгладени числа, не едно шумно
    # тримесечие срещу една плъзгаща.
    # Живо проверено 29.07.2026: n=125 от 1995Q1, последно 2026Q1.
    "BG_NGDP": {
        "source": "eurostat",
        "id": "namq_10_gdp?geo=BG&unit=CP_MEUR&na_item=B1GQ&s_adj=NSA",
        "region": "BG",
        "name_bg": "Номинален БВП (г/г, 4-тримесечна плъзгаща)",
        "name_en": "Nominal GDP (YoY, 4Q rolling)",
        "lens": [],
        "context_only": True,
        "peer_group": "context",
        "tags": ["context"],
        ...
        "typical_release": "mid_quarter",
        "revision_prone": True,
        "narrative_hint": "Знаменателят на кредитния въпрос — темпът, над който "
                          "кредитът надува баланси, а не производство. Контекст: "
                          "не влиза в композита.",
    },
    ...
    # Не се фетчва: ражда се от снапшота в `sources/derived.py::derive_series`
    # СЛЕД сплайса на кредитните серии. Конвенцията и ревизионната ѝ история
    # живеят в докстринга на онзи модул — това е модулната памет, не тук.
    # Накратко: HH сток към края на тримесечие Q ÷ 4Q СУМА на номиналния БВП до
    # СЪЩОТО Q × 100; само тримесечия с ДВЕТЕ страни налични. Точно това
    # изравняване прави невъзможна грешката, която ръчната сверка правеше —
    # сдвояване на най-свежия кредитен сток с по-стар БВП.
    # Показва се като НИВО в проценти (`transform: level`, `is_rate: True`) —
    # серията вече Е съотношение.
    "BG_CREDIT_GDP_HH": {
        "source": "derived",
        # „id"-то на изведена серия е РЕЦЕПТАТА ѝ, не адрес: тя няма собствен
        # първоизточник (съставките си имат). `source_url` го знае и не строи
        # линк — по-добре без линк, отколкото с линк, който лъже.
        "id": "BG_LOANS_HH ÷ Σ4Q(BG_NGDP) × 100",
        "region": "BG",
        "name_bg": "Кредит домакинства / БВП (4Q сума)",
        "name_en": "Household Credit to GDP (4Q sum)",
        "lens": [],
        "context_only": True,
        "peer_group": "context",
        "tags": ["context", "housing", "derived"],
        "transform": "level",
        "is_rate": True,
        "historical_start": "2005-12-01",
        "release_schedule": "quarterly",
        "typical_release": "mid_quarter",
        "revision_prone": True,
        "narrative_hint": "Кредитната интензивност на домакинствата — колко от "
                          "годишната икономика тежи ипотечно-потребителският им "
                          "дълг. Дискриминаторът на ливъриджната хипотеза; "
                          "контекст, не влиза в композита.",
    },
