...
# ── БАЛОННАТА ДВОЙКА (мандат №53, дефиницията замразена в П4 §5) ─────────────
# Представителите не са избор на вкус, а провенанс:
#   · имоти = `BG_HPI` — ЦЕНАТА НА АКТИВА. `BG_PERMITS` е тръбата (предлагането
#     след година-две), т.е. друг въпрос — затова не е представител тук.
#   · кредит = `BG_LOANS_HH` ИЛИ `BG_LOANS_NFC` — двата заема дават ЕДИН
#     кредитен сигнал (peer-прецедентът на групата `lending` в каталога),
#     затова е достатъчно да гори единият.
BUBBLE_PAIR_PROPERTY = "BG_HPI"
BUBBLE_PAIR_CREDIT = ("BG_LOANS_HH", "BG_LOANS_NFC")
...
BUBBLE_PAIR_PROVENANCE = (
    "П4 ретро-изпит (49-P4-TENSION-REPORT.md, замразената история): двойката "
    "свети 8/8 в бум-прозореца 2007-08 и 0/20 в спокойните 2015-19 (25 "
    "запалени реда общо). Показанието е сигнал за СЪ-ПРЕГРЯВАНЕ на цената на "
    "актива и кредита — НЕ прогноза и НЕ етикет на тензия (К1 е отделен слой). "
    "Старият К3 етикет („≥2 двойки“ върху лещови score-ове) е изпитан и "
    "ПЕНСИОНИРАН с П4: изроден (0 пъти на 83 реда) и обърнат (единствените му "
    "активирания са в СПОКОЙНИТЕ 2015-19). Прероденото стъпва на `temp_hot`, "
    "не на score-ове."
)
