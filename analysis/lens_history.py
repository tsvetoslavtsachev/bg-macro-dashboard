...
1. **Реконструираната история** (`data/lens_history.parquet`) — какво би казал
   ...
   **днешните дефиниции** (днешният каталог, днешните полярности, днешните
   ...
   какво е показвал екранът тогава, защото:
     ...
     · съставът на уреда (6 лещи, 17 серии) не е съществувал през 2007.
...
# ── Решетката ────────────────────────────────────────────────────────────────
# Раждането на кредитния seed (БНБ „Кредитна динамика", 2005Q4). По-рано
# кредитната леща пада до 1 серия и „историята" би мерила ДРУГ уред — затова
# решетката не тръгва от 2000-та само защото Eurostat стига дотам.
GRID_START = "2005-12-01"
...
HONESTY_LABEL = (
    ...
    "тримесечна решетка от 2005Q4; не point-in-time запис."
)
