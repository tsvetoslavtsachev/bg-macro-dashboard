...
SIX_TO_SEVEN_NOTE = (
    "П4 тестът съдеше 6-лещовата история; вграденият К1 тече на 7-лещовия "
    "уред — числата не се сравняват 1:1 с доклада."
)
...
def anchors_note(history: Optional[Any] = None) -> str:
    ...
    base = (
        f"Котвите на прочита идват от П4 ({P4_REPORT}) и са мерени на "
        f"6-лещовата история: спокойните {epoch_label()} ≈ {P4_CALM_RATIO:.3f} "
        f"(тогавашният висок композит беше ИСТИНСКИ консенсус) срещу "
        f"{P4_PEAK_LABEL} = {P4_PEAK_RATIO:.3f} — „фалшивата неутралност“. "
        f"{SIX_TO_SEVEN_NOTE}"
    )
