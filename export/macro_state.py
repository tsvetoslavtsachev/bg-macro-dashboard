...
=====================
Машинният api export — `output/api/macro_state.json` (мандат ORGANISM-v1;
bg огледалото на jp Ф1 — схемата се пише веднъж, подписано решение №1).
...
парсват Markdown. Този модул е bg носителят на схемата на новата генерация
(`bg-macro-state v1`; jp носи `jp-macro-state v1` + йена-слоя си).
...
`regime` dict; латинският ключ — от `config.REGIME_KEYS`.
